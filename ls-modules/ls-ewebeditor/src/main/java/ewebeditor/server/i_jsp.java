...
public class i_jsp {
    ...
    private String sWordEqVersion;
    private String sWordEqLicense;
    ...
    public final void Load(PageContext pagecontext) throws ServletException, IOException {
        ...
        sWordEqVersion = myUtil.getConfigString("WordEqVersion", sConfig);
        sWordEqLicense = myUtil.getConfigString("WordEqLicense", sConfig);

        String sAction = myUtil.dealNull(m_request.getParameter("action")).toUpperCase();
        if (sAction.equals("LICENSE")) {
            ShowLicense();
        } else if (sAction.equals("WORDEQLICENSE")) {
            ShowWordEqLicense();
    ...
    private void ShowWordEqLicense() throws IOException {
        ...
        String ret = "";
        String[] aa = myUtil.split(sWordEqLicense, ";");
        for (int i = 0; i < aa.length; i++) {
            String[] a;
            String s_Sep = "";
            if (aa[i].indexOf(",") > 0) {
            ...
            if (a.length == 4) {
                if (a[3].length() == 32) {
                    boolean b = false;
                    if (a[0].equals("3")) {
                        if (s_Domain.equals(a[2])) {
                            b = true;
                        } else if (s_Domain.length() > a[2].length()) {
                            if (s_Domain.substring(s_Domain.length() - a[2].length() - 1).equals("." + a[2])) {
                                b = true;
                            }
                        }
                    } else {
                        if ((s_Domain.equals(a[2])) || (s_Domain.equals("www." + a[2]))) {
                            b = true;
                        }
                    }
                    ...
                    if (b) {
                        ...
                        ret = ret + myUtil.MD5(a[3].substring(0, 16) + r, 16) + myUtil.MD5(a[3].substring(16) + r, 16);
                        break;
                    }
                }
            }
    ...
    private void ShowConfig() throws IOException {
        ...
        String s_FSPath = "", s_FSDomain = "", s_FSL = "";
        s_FSPath = aTmpStyle[112];
        if (!s_FSPath.equals("")) {
            s_FSDomain = GetDomainFromUrl(s_FSPath);
            if (!s_FSDomain.equals("")) {
                if (myUtil.CheckLicense(s_FSDomain, sLicense)) {
                    s_FSL = "ok";
                }
            }
        }
        ...
        ret = ret + "config.WordEqVersion = \"" + sWordEqVersion + "\";" + "\r\n";
        ...
        m_response.setContentType("application/x-javascript");
        m_out.print(ret);
    }
