...
public class style_jsp {
    ...
    String sCodeFormat = "", sTB2Flag = "", sTB2Code = "", sTB2Edit = "", sTB2Text = "", sTB2View = "", sTB2Max = "", sShowBlock = "";
    ...
    String sAutoDoneWordPaste = "", sAutoDoneExcelPaste = "", sAutoDoneQuickFormat = "", sAutoDonePasteOption = "";
    String sWordEqImport = "", sResMovePath = "", sFileServerPath = "";
    ...
    public final void Load(PageContext pagecontext) throws ServletException, IOException {
        ...
        if (myUtil.sAction.equals("STYLEADD") || myUtil.sAction.equals("STYLESET")) {
            ...
            String s_FormTB2Flag, s_FormTB2Code, s_FormTB2Edit, s_FormTB2Text, s_FormTB2View, s_FormTB2Max;
            ...
            String s_FormAutoDoneWordPaste, s_FormAutoDoneExcelPaste, s_FormAutoDoneQuickFormat, s_FormAutoDonePasteOption;
            String s_FormWordEqImport;
            ...
            if (myUtil.sAction.equals("STYLEADD")) {
                ...
                sResMovePath = myUtil.htmlEncode(sResMovePath);
                sFileServerPath = myUtil.htmlEncode(sFileServerPath);
                s_Title = "设置样式";
            ...
            s_FormTB2Code = myUtil.InitCheckBox("d_tb2code", "1", sTB2Code);
            s_FormTB2Edit = myUtil.InitCheckBox("d_tb2edit", "1", sTB2Edit);
            s_FormTB2Text = myUtil.InitCheckBox("d_tb2text", "1", sTB2Text);
            s_FormTB2View = myUtil.InitCheckBox("d_tb2view", "1", sTB2View);
            ...
            s_FormAutoDonePasteOption = myUtil.InitCheckBox("d_autodonepasteoption", "1", sAutoDonePasteOption);
            ...
            s_FormWordEqImport = myUtil.InitSelect("d_wordeqimport", myUtil.split("启用|不启用", "|"), myUtil.split("1|0", "|"), sWordEqImport, "", "");

            s_FormAutoDetectLanguage = myUtil.InitSelect("d_autodetectlanguage", myUtil.split("自动检测|不自动检测", "|"), myUtil.split("1|0", "|"), sAutoDetectLanguage, "", "");
            s_FormDefaultLanguage = myUtil.InitSelect("d_defaultlanguage", myUtil.split("简体中文|繁体中文|英文|日语|西班牙语|俄语|德语|法语|意大利语|荷兰语|瑞典语|葡萄牙语|挪威语|丹麦语", "|"), myUtil.split("zh-cn|zh-tw|en|ja|es|ru|de|fr|it|nl|sv|pt|no|da", "|"), sDefaultLanguage, "", "");
            ...
            s_FormUploadObject = myUtil.InitSelect("d_uploadobject", myUtil.split("jspSmartUpload|commons fileupload|struts2|struts2.5|webwork2", "|"), myUtil.split("0|1|2|4|3", "|"), sStyleUploadObject, "", "");
            ...
            m_out.print("<table border=0 cellpadding=0 cellspacing=1 align=center class=form>" +
                ...
                "<tr><td>限宽模式宽度：</td><td><input type=text class=input size=20 name=d_fixwidth title='留空表示不启用，可以填入如：500px' value=\"" + sFixWidth + "\"></td><td>界面皮肤目录：</td><td><input type=text class=input size=15 name=d_skin title='存放界面皮肤文件的目录名，必须在skin下' value=\"" + sSkin + "\"> <select size=1 id=d_skin_drop onchange='this.form.d_skin.value=this.value'><option>-系统自带-</option><option value='blue1'>blue1</option><option value='blue2'>blue2</option><option value='flat1'>flat1</option><option value='flat2'>flat2</option><option value='flat3'>flat3</option><option value='flat4'>flat4</option><option value='flat5'>flat5</option><option value='flat6'>flat6</option><option value='flat7'>flat7</option><option value='flat8'>flat8</option><option value='flat9'>flat9</option><option value='flat10'>flat10</option><option value='green1'>green1</option><option value='light1'>light1</option><option value='office2000'>office2000</option><option value='office2003'>office2003</option><option value='officexp'>officexp</option><option value='red1'>red1</option><option value='vista1'>vista1</option><option value='yellow1'>yellow1</option></select> <span class=red>*</span></td></tr>" +
                ...
                "<tr><td>WordEQ数据导入：</td><td>" + s_FormWordEqImport + " <span class=red>*</span></td><td>资源文件移动路径：</td><td><input type=text class=input size=20 name=d_resmovepath value=\"" + sResMovePath + "\"></td></tr>" +
                ...
                "<tr><td>路径模式：</td><td>" + s_FormBaseUrl + " <span class=red>*</span> <a href='#baseurl'>说明</a></td><td>上传路径：</td><td><input type=text class=input size=35 name=d_uploaddir title='上传文件所存放路径，相对eWebEditor根目录文件的路径' value=\"" + sStyleUploadDir + "\"> <span class=red>*</span></td></tr>" +
                "<tr><td>显示路径：</td><td><input type=text class=input size=35 name=d_basehref title='显示内容页所存放路径，必须以&quot;/&quot;开头' value=\"" + sStyleBaseHref + "\"></td><td>内容路径：</td><td><input type=text class=input size=35 name=d_contentpath title='实际保存在内容中的路径，相对显示路径的路径，不能以&quot;/&quot;开头' value=\"" + sStyleContentPath + "\"></td></tr>" +
                "<tr><td>文件服务接口路径：</td><td><input type=text class=input size=35 name=d_fileserverpath title='' value=\"" + sFileServerPath + "\"></td><td>&nbsp;</td><td>&nbsp;</td></tr>" +
            ...
            sAutoDetectLanguage = myUtil.dealNull(m_request.getParameter("d_autodetectlanguage"));
            sDefaultLanguage = myUtil.dealNull(m_request.getParameter("d_defaultlanguage"));
            ...
            sTB2Code = myUtil.dealNull(m_request.getParameter("d_tb2code"));
            sTB2Edit = myUtil.dealNull(m_request.getParameter("d_tb2edit"));
            sTB2Text = myUtil.dealNull(m_request.getParameter("d_tb2text"));
            sTB2View = myUtil.dealNull(m_request.getParameter("d_tb2view"));
            ...
            sAutoDonePasteOption = myUtil.dealNull(m_request.getParameter("d_autodonepasteoption"));
            sWordEqImport = myUtil.dealNull(m_request.getParameter("d_wordeqimport"));
            sResMovePath = myUtil.dealNull(m_request.getParameter("d_resmovepath"));
            sFileServerPath = myUtil.dealNull(m_request.getParameter("d_fileserverpath"));
            ...
            if (myUtil.sAction.equals("STYLEADDSAVE")) {
                ...
                if (!myUtil.WriteConfig(myUtil.sLicense, myUtil.sUsername, myUtil.sPassword, myUtil.aStyle, myUtil.aToolbar)) {
                    m_out.print(myUtil.getError("Error: getRealPath"));
                    return;
                }
                ;
            ...
            for (int nLoop = 0; nLoop < 1; nLoop++) {
                ...
                m_out.print("<script language='javascript' src='../language/zh-cn.js'></script>");
