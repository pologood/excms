 ...
 * 构造一个新的图实例。注意,构造函数不需要把一个容器,因为图实例创建UI反过来将创建的容器图。因此,容器在EditorUi后面分配。
 */
Graph = function(container, model, renderHint, stylesheet)
{
	...
	// 是否允许点击中心的关联图标时自动生成关联
	this.allowAutoPanning = false;
	
	this.setInvokesStopCellEditing(false);
...
var  ATTRWIN=null,ATTRWINObj = null;

function openAttrWin(url,name){
	w =  440;
	h =  570;
  //var frame = document.getElementById('attrWin');
	var _w = document.body.clientWidth;
	var _h = (document.body.clientHeight || document.documentElement.clientHeight);
  /*
  if(!frame){
		frame = document.createElement('iframe');
		frame.setAttribute('id', 'attrWin');
	...
	*/
	...
	ATTRWINObj = Ls.dialog.get();
	//console.log('ATTRWINObj >>>');
	//console.log(ATTRWINObj);
	//console.log('ATTRWIN >>>');
	//console.log(ATTRWIN);
	if(ATTRWINObj!=null)
		ATTRWIN=ATTRWINObj.get('ATTRWIN',1);
	//console.log();
	if(ATTRWIN!=null){	
		Ls.log.add('ATTRWIN >>>');
		Ls.log.add(ATTRWIN);
		if(ATTRWIN.iframe){
			ATTRWIN.title("设置["+name+"]属性");
			ATTRWIN.setContent("url:"+url,false,true);	
			ATTRWIN.show();		
		}else{
			__createAttrWin(url,w,h,45,_w-w-10,name);
		}
			//ATTRWIN.content = "url:"+url;		
			//ATTRWIN.iframe.src = url;
			
		//ATTRWIN = new mxWindow('设置属性', frame, (_w-w-20), 76, w+8, h+28);
		//ATTRWIN.setMaximizable(true);
		//ATTRWIN.setResizable(true);
	
	}else{
		__createAttrWin(url,w,h,45,_w-w-10,name);
		
	}
    //if(!ATTRWIN.isVisible){
		//ATTRWIN.setVisible(true);	
	
	//ATTRWIN.show();	
}
function __createAttrWin(url,width,height,top,left,name){
	name = "设置[" + name + "]属性" || "设置属性";
	...
	Ls.dialog.create({
		...
		close:function(obj){
            ...
            if (obj) {
                ...
                //editorUi.editor.updateCell(obj.id, obj.name);
            }
        },
		resize:true							
	},function(){
