...
Editor = function()
{
	...
	this.init(); // 执行初始化方法
	this.initStencilRegistry(); // 初始化模板注册
	this.graph = new Graph(); // 实例化一个Graph对象
	this.outline = new mxOutline(this.graph); //增加预览模块
	this.outline.updateOnPan = true;
	this.undoManager = this.createUndoManager(); // 创建撤消管理器
	this.status = '';
	...
	*/
	...
	this.graph.getSelectionModel().addListener(mxEvent.CHANGE,function(){
        ...
        if (this.cells && this.cells.length == 1) {
            //console.log(this.cells[0].id);
            var cell = this.cells[0];
            var aid = cell.activityId;
            var atype = cell.activityType; //1-开始;100--结束;22-标准活动;3-路由活动;4-转移线设置
            ...
            var url = openAttrURL + '&id=' + aid + "&atype=" + atype + '&activityName=' + encodeURIComponent(name) + '&etc=' + new Date().getTime();
            //alert(name)
            // 只有当活动类型为0,2时才弹出属性设置窗口
            if (atype && atype == '4') {//路由
                var _target = cell.target;
                var _source = cell.source;
                if (_target != null && _source != null && !(_target.activityId < 1 && _source.activityId < 1)) {
                    url = openTransitionAttrURL + '&id=' + aid + '&fromId=' + _source.activityId + '&toId=' + _target.activityId + '&text=' + encodeURIComponent(name) + '&etc=' + new Date().getTime();
                } else {
                    hideAttrWin();
                    return;
                }
...
　　var keyHandler = new mxKeyHandler(this.graph);
	var __graph = this.graph;
    keyHandler.bindKey(46, function (evt) {
    var cells = __graph.getSelectionCells();
    if (cells) {
        for (var i = 0, len = cells.length; i < len; i++) {
            var cell = cells[i];
            var atype = cell.activityType,
                aid = cell.activityId;
            //console.log('activityType = ' + type);
            // 开始节点\结束节点不允许删除
            if ((atype && (atype == 1)) && aid > 0) {
                alert("开始节点不允许删除!");
            } else {
                __graph.removeCells([cell]);
            }
        }
    }});
	keyHandler.bindKey(27, function (evt) {try{}catch(e){}});
	/*this.graph.addListener(mxEvent.CLICK, function(sender, evt)
    {
		console.log("mxEvent.CLICK >>> ");
     	SELECT_MODEL = evt.getProperty('cell');
...
* 更新相关的cell
*/
Editor.prototype.updateCell = function(aid,name){

	console.log('----------11--------Editor.prototype.updateCell >>>===' + name);
	//console.log();
	var model = this.graph.getModel();
	//console.log('this.graph.getModel() >>>');
	//console.log(model);
	//console.log('params aid = '+ aid + ', name = '+ name);
	if(!SELECT_MODEL)return;
	//var value = model.getValue(SELECT_MODEL);
	//console.log('model.getValue(SELECT_MODEL) >>>');
	//console.log(value);
	//console.log('mxUtils.isNode(value) = ' + mxUtils.isNode(value));
	//if (mxUtils.isNode(value)){
		...
		try
		{
			var cell = model.getCell(SELECT_MODEL.getId())
			//console.log('model.getCell(SELECT_MODEL.getId()) >>>');
			//console.log(cell);
			cell.setValue(name);
			//console.log('cell.setValue(name) >>>');
			//console.log(cell);
			
			model.setValue(cell, name);
			if(cell.activityType!=0 || cell.activityType!=100){
				cell.activityId=aid;
			}
			//model.execute(new mxValueChange(this, cell, name));
			//model.setValue(cell,'value',name);
			//var c1 = new mxCellAttributeChange(SELECT_MODEL, 'value',name);
			//model.execute(c1);
			//c1 = new mxCellAttributeChange(SELECT_MODEL, 'activityId',aid);
			//model.execute(c1);			
 ...
 * 将xml文件导入到当前编辑器的graph中
 * xml文件内容格式如: <mxGraphModel><root>...</root></mxGraphModel>
 */
...
Editor.prototype.init = function()
{
	...
	// 如果为IE6则加载兼容样式
	if (mxClient.IS_IE6 || mxClient.IS_IE7)
	{
