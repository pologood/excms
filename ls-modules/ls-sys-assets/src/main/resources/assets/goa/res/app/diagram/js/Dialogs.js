...
function ExportDialog(editorUi)
{
	...
	var table = document.createElement('table');
	table.setAttribute('class', 'tform');
	var tbody = document.createElement('tbody');
	...
	imageFormatSelect.setAttribute('class', 'select');
	...
	widthInput.setAttribute('class', 'text');
	...
	heightInput.setAttribute('class', 'text');
	...
	borderInput.setAttribute('class', 'text');
	...
	function formatChanged()
	{
		...
		if (imageFormatSelect.value === 'xml')
		{
			...
			widthInput.setAttribute('class', 'text-disabled');
			heightInput.setAttribute('disabled', 'true');
			heightInput.setAttribute('class', 'text-disabled');
			borderInput.setAttribute('disabled', 'true');
			borderInput.setAttribute('class', 'text-disabled');
		}
		else
		{
		...
		else
		{
	        ...
	        if (format == 'xml')
	    	{
	        	var xml = encodeURIComponent(mxUtils.getXml(editorUi.editor.getGraphXml()));        	
						//new mxXmlRequest(SAVE_URL, 'filename=' + escape(name) + '&xml=' + xml).simulate(document, "_blank");
						new mxXmlRequest(SAVE_URL, 'filename=' + escape(name) + '&xml=' + xml).simulate(document, "");
	    	}
	        else if (format == 'svg')
	    	{
				...
				if (xml.length < MAX_REQUEST_SIZE)
				{
					...
					//new mxXmlRequest(SAVE_URL, 'filename=' + escape(name) + '&format=' + format +
	        ...
	        else
	        {
					...
					height < MAX_HEIGHT && height > 0)
				{
					...
					new mxXmlRequest(EXPORT_URL, 'filename=' + escape(name) + '&format=' + format +
