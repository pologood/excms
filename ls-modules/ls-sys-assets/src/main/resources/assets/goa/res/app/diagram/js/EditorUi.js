 ...
 */
EditorUi.prototype.vsplitPosition = 0;
...
{
	//Ls.log.add('EditorUi.js EditorUi.prototype.init >>>');
...
EditorUi.prototype.refresh = function()
{
	...
	this.sidebarContainer.style.top = (this.menubarHeight + this.toolbarHeight +3) + 'px';
	...
	if (quirks)
	{
		...
		var sidebarHeight = (h - effVsplitPosition - this.splitSize - this.footerHeight - this.menubarHeight - this.toolbarHeight-3);
		...
		var diagramHeight = (h - this.footerHeight - this.menubarHeight - this.toolbarHeight -3);
...
EditorUi.prototype.createDivs = function()
{
	...
	//console.log($(container));
	var hstr='<h3>工作流设计器</h3>';
	//container.parent().html(hstr);
	//$(container).html(hstr);
	this.menubarContainer.innerHTML = hstr;
...
/**
 * 保存xml到服务器
 * 此处应对应相关的服务器接口,保存成功后返回状态值
 */
EditorUi.prototype.saveFile = function()
{

	//mxLog.info('saveFile start >>>');
	...
	//mxLog.info(xml);
	//console.log(xml);
	//mxLog.info('saveFile end >>>');
	//console.log('saveFile end >>>');

};
EditorUi.prototype.saveFileToLocal = function(forceDialog)
{
