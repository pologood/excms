	...
	public void beat(){
		TimerTask task=new TimerTask() {
			...
			public void run() {
				...
				FileServerPoolSysout.warn("ConnectionPool execution HeartBeat to fdfs server");
				for(int i=0;i<idleConnectionPool.size();i++){
					try {
						ts=idleConnectionPool.poll(waitTimes, TimeUnit.SECONDS);
						if(ts!=null){
                            cn.lonsun.common.upload.csource.fastdfs.ProtoCommon.activeTest(ts.getSocket());
