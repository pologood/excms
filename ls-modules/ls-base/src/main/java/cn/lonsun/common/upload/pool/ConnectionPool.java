...
public class ConnectionPool {
	...
	private void init() {
		...
		try {
			...
			for (int i = 0; i < poolSize; i++) {
                ...
                cn.lonsun.common.upload.csource.fastdfs.ProtoCommon.activeTest(trackerServer.getSocket());
	...
	public TrackerServer checkout(int waitTimes) throws InterruptedException {
		...
		if (client1 == null) {
			FileServerPoolSysout.warn("FileServerPool wait time out ,return null");
			throw new NullPointerException("FileServerPool wait time out ,return null");
		}
	...
	public synchronized void drop(TrackerServer trackerServer) {
		...
		if (busyConnectionPool.remove(trackerServer) != null) {
			try {
				FileServerPoolSysout.warn("FileServerPool drop a connnection");
				FileServerPoolSysout.warn("FileServerPool size:"
                        + (busyConnectionPool.size() + idleConnectionPool.size()));
				TrackerClient trackerClient = new TrackerClient();
				trackerServer = trackerClient.getConnection();
			} catch (IOException e) {
				trackerServer = null;
				FileServerPoolSysout.warn("FileServerPool getConnection generate exception");
				e.printStackTrace();
			} finally {
				...
				try {
					...
					idleConnectionPool.add(trackerServer);
					FileServerPoolSysout.warn("FileServerPool add a connnection");
					FileServerPoolSysout.warn("FileServerPool size:"
                            + (busyConnectionPool.size() + idleConnectionPool.size()));
				} catch (IOException e) {
					e.printStackTrace();
			...
			public void run() {
				...
				while (true) {
					...
					try {
						...
						FileServerPoolSysout.warn("current FileServerPool has size:"
...
//					FileServerPoolSysout.warn("busyConnectionPool start close trackerserver");
...
//				}
				...
				if(idleConnectionPool.size()!=0){
					FileServerPoolSysout.warn("idleConnectionPool start close trackerserver");
					FileServerPoolSysout.warn(msg);
