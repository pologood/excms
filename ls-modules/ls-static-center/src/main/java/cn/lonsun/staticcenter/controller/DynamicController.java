...
public class DynamicController extends BaseController {
    ...
    public Object getHtml(@PathVariable String module, @PathVariable String action, @PathVariable long id,
        ...
        //模块编码获取到之后进行过滤
        if(null != module){
            module =  HtmlRegexpUtil.Replace(module,"<","");
            module =  HtmlRegexpUtil.Replace(module,">","");
            module =  HtmlRegexpUtil.Replace(module,"\"","");
            module =  HtmlRegexpUtil.Replace(module,"\'","");
            module =  HtmlRegexpUtil.Replace(module,"%","");
        ...
        // 设置是否WAP访问
        // context.setFrom(isFromMobile ? Context.From.WAP.toString() :
        // Context.From.PC.toString());

        // 跟线程绑定
        ContextHolder.setContext(context.setSource(MessageEnum.CONTENTINFO.value()));// 默认内容管理
        // 判断jsonp请求
        String callback = paramMap.get("jsonpCallback");
        Object obj = service.queryHtml(action, id, context.setModule(module).setAction(action).setParamMap(paramMap));
