...
public class LotteryDrawController extends BaseController {
    ...
    public String drawForm(String name,String phone,Model model,Long titleNum) {
        ...
        if(null!=questionsEO.getAnswerEOs()||questionsEO.getAnswerEOs().size()>0){
            for(LotteryAnswerEO answerEO :  questionsEO.getAnswerEOs()){
                if(answerEO.getAnswerId().equals(questionsEO.getAnswerId())){
                    model.addAttribute("option",answerEO.getAnswerOption());
