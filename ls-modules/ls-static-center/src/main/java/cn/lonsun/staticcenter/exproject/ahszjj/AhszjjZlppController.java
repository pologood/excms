 ...
 * 质量品牌升级工程专题-综合信息.质量工程
 */
@Controller
@RequestMapping("/ahszjj/zlpp")
public class AhszjjZlppController extends BaseController {
    //政民互动导入页面
    private final String PAGE_News = "/ahszjj/zlpp/impListPage";
    //政民互动导入
    private final String ACTION_News = "/ahszjj/zlpp/newsImp";

    private static JdbcUtils loadRemoteJdbc() {
        JdbcUtils jdbcUtils = JdbcUtils.getInstance();
        ////10.0.0.253:1433
      /*  jdbcUtils.setURLSTR("jdbc:jtds:sqlserver://61.191.61.136:31433;DatabaseName=szjjzty;useLOBs=false;");
        jdbcUtils.setUSERNAME("szjjzty");
        jdbcUtils.setUSERPASSWORD("szjjzty");*/

        jdbcUtils.setURLSTR("jdbc:jtds:sqlserver://".concat(AhszjjConstant.db_zlpp_IP).concat(":" + AhszjjConstant.db_zlpp_Port)
                .concat(";DatabaseName=".concat(AhszjjConstant.db_zlpp_name).concat(";useLOBs=false;")));
        jdbcUtils.setUSERNAME(AhszjjConstant.db_zlpp_user);
        jdbcUtils.setUSERPASSWORD(AhszjjConstant.db_zlpp_pwd);
