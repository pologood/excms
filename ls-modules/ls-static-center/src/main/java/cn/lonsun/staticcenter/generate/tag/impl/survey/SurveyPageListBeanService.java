...
@Component
public class SurveyPageListBeanService extends AbstractBeanService {
        ...
        if (null != pagination) {
            ...
            if (null != resultList && !resultList.isEmpty()) {
                for (Object o : resultList) {
                    ...
                    if (vo.getIsLink() == 0) {
                        vo.setLinkUrl(PathUtil.getLinkPath(HtmlEnum.CONTENT.getValue(), HtmlEnum.ACRTILE.getValue(), vo.getColumnId(), vo.getContentId())+"?type=1");
                        vo.setViewUrl(PathUtil.getLinkPath(HtmlEnum.CONTENT.getValue(), HtmlEnum.ACRTILE.getValue(), vo.getColumnId(), vo.getContentId())+ "?type=0");
                    }else{
                        vo.setViewUrl(vo.getLinkUrl());
                    }
                }
            }
            // 设置连接地址
            String path = PathUtil.getLinkPath(HtmlEnum.CONTENT.getValue(), HtmlEnum.COLUMN.getValue(), columnId, null);// 拿到栏目页和文章页id
    ...
    // 获取对象
//    private Object getSurveyInfo(JSONObject paramObj) {
//        Context context = ContextHolder.getContext();
