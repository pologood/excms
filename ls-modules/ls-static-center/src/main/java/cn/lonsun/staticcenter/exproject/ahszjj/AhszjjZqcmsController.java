 ...
 */
@Controller
@RequestMapping("/ahszjj/zqcms")
public class AhszjjZqcmsController extends BaseController {
    //政民互动导入页面
    private final String PAGE_News = "/ahszjj/zqcms/impListPage";
    //政民互动导入
    private final String ACTION_News = "/ahszjj/zqcms/newsImp";
    ...
    private static JdbcUtils loadRemoteJdbc() {
       ...
       /* jdbcUtils.setURLSTR("jdbc:jtds:sqlserver://61.191.61.136:31433;DatabaseName=ahzjj_qzlx;useLOBs=false;");
        ...
        jdbcUtils.setURLSTR("jdbc:jtds:sqlserver://".concat(AhszjjConstant.db_zqcms_IP).concat(":" + AhszjjConstant.db_zqcms_Port)
                .concat(";DatabaseName=".concat(AhszjjConstant.db_zqcms_name).concat(";useLOBs=false;")));
        jdbcUtils.setUSERNAME(AhszjjConstant.db_zqcms_user);
        jdbcUtils.setUSERPASSWORD(AhszjjConstant.db_zqcms_pwd);
        return jdbcUtils;
    }
    ...
    public ModelAndView impListPage(String siteId){
        ...
        String type_title = "纤维检验局";
        String imp_action = ACTION_News;
        SyncZqcmsArticleNews articleNews = new SyncZqcmsArticleNews(jdbcUtils, site_id);
        //原站点栏目
    ...
    public String newsImp(Long siteId,String fromCode,String indicatorIdAndCode) {
        ...
        if(StringUtils.isEmpty(fromCode) || StringUtils.isEmpty(indicatorIdAndCode)){
            return "未知操作!<a href=\"".concat(PAGE_News).concat("\">返回</a>");
        }
        JdbcUtils jdbcUtils = loadRemoteJdbc();
        ...
        SyncZqcmsArticleNews articleNews = new SyncZqcmsArticleNews(jdbcUtils,siteId, AhszjjConstant.createUserId, indicatorId);
