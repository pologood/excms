var unitManage_all = function () {
    ...
    var getUnit = function () {
        ...
        var getData = function () {
            return Ls.ajax({
                url: "/contentModel/getOrgansBySite",
                success: function (json) {
                    if (json.data == null) {
                        return;
                    }
                    var data = json.data;
                    if (!Ls.isEmpty(recDepartIds)) {
                        recDepartIds = recDepartIds + ",";
                        for (var i = 0, l = data.length; i < l; i++) {
                            if ((recDepartIds).indexOf(data[i].organId + ",") >= 0) {
                                data[i].checked = true;
                            }
                        }
                    }
                    Ls.treeDataFilter(data, Ls.treeDataType.UNIT);
